...
export default function Verify() {
  ...
  useEffect(() => {
    if (!firebaseUser || !token || running || result || error) return;
    setRunning(true);
    verifyToken(token, firebaseUser).then(setResult).catch((err) => setError(err.message)).finally(() => setRunning(false));
  }, [firebaseUser, token, running, result, error]);
