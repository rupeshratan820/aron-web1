...
export default function AppShell({ children }) {
  ...
  const { identity, firebaseUser, login, logout } = useAuthStore();
  ...
  return (
    <div className="min-h-screen overflow-hidden">
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 sm:px-6 lg:px-8">
          ...
          <div className="hidden items-center gap-3 lg:flex">
            {firebaseUser ? (
              ...
              <Button onClick={login} icon={LogIn}>Login</Button>
            )}
          </div>
        ...
        {open ? (
          <div className="border-t border-white/10 px-4 py-3 lg:hidden">
            <div className="grid gap-2">
              ...
              <Button onClick={firebaseUser ? logout : login} icon={firebaseUser ? LogOut : LogIn}>{firebaseUser ? "Logout" : "Login"}</Button>
            </div>
          </div>
        ) : null}
      </header>
      ...
      <footer className="relative z-10 border-t border-white/10 px-4 py-8 text-center text-sm text-white/50">
        ...
        Aron uses Firebase client rules, Discord OIDC login, and hashed verification artifacts.
      </footer>
    </div>
  );
}
