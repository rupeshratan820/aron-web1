...
export function discordAvatar(user) {
  const provider = user?.providerData?.find((entry) => entry.providerId?.includes("discord"));
  const photoURL = user?.photoURL || provider?.photoURL;
  return photoURL || `https://api.dicebear.com/8.x/shapes/svg?seed=${encodeURIComponent(user?.uid || "aron")}`;
}

export function getDiscordIdentity(user) {
  const provider = user?.providerData?.find((entry) => entry.providerId?.includes("discord")) || user?.providerData?.[0];
  const discordId = provider?.uid || user?.reloadUserInfo?.screenName || user?.uid;
  return {
    discordId: String(discordId || ""),
    username: provider?.displayName || user?.displayName || "Aron Player",
