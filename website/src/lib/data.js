...
import { getDiscordIdentity } from "./format.js";
...
export async function syncWebUser(firebaseUser) {
  const identity = getDiscordIdentity(firebaseUser);
...
export async function verifyToken(rawToken, firebaseUser) {
  const identity = getDiscordIdentity(firebaseUser);
  const discordId = identity.discordId;
  if (!rawToken) throw new Error("Missing verification token.");
  if (!discordId) throw new Error("Discord identity was not available from Firebase Auth.");
